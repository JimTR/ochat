...
$time = "1644129784";
    ...
    function printr($var,$return,$web,$key="")
    {
		//if ($web === false ) {echo 'web is false'.cr; }
		if( $web === false) {
			$output='';
			foreach($var as $k=>$v){
				// parse array
			...
			if ($return === true){
			        return  "<div id='printr' style='font-family:courier;'>$output</div>";
			   }
			if ($return === false) {
				echo $output;
		}
    ...
    function time2str($ts,$format=false)
    {
        if(!ctype_digit($ts))
            $ts = strtotime($ts);
        ...
        elseif($diff > 0)
        {
			...
			if ($ret > -12) {
				$ret = 0 - $ret;
				if ($ret ==1 ) { $ret = "last month ";}
				else {$ret = "$ret months ago";}
			}
			else {
				$ret = 0 - $ret;
				$years = floor($ret / 12);
				...
				else { $year_word = " years";}
				$ret = "$years $year_word,  $month months ago";
			}
			if ($format){ $ret = date('M Y', $ts); }// just year &  month
    ...
    function geturl($url, $username = null, $password = null,$options= null,$query=null)
    {
        if(function_exists('curl_init'))
        {
            ...
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
            curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 10); //curl timeout
            $html = curl_exec($ch);
            $curl_errno = curl_errno($ch);
			$curl_error = curl_error($ch);
			if ($curl_errno) {log_to(LOG,"Error $curl_errno $curl_error from $url");}
            curl_close($ch);
            return $html;
        }
...
function log_to ($file,$info,$append=FILE_APPEND)
	{
	if(is_array($info)) {return;}	
	if (!strrpos ($info , "\r\n" )){ $info .="\r\n";}
		file_put_contents($file, $info, $append );
		chmod($file, 0666); 
	}
function getip()
	{
...
function array_findo($needle, array $haystack)
{
...
function convert_to_argv ($type,$arraytype ='',$retain=false) 
{
	...
	foreach  ($type as $key => $value) {
		...
		if ($value == trim($value) && str_contains($value, ' ')) {
			// check if the string is encapsulated by quotes, if not add them ;
			if(!str_starts_with($value,'"') or !str_ends_with($value,'"')) {$value = "'$value'";}
		}
		 if ($retain === true ) {
			$cmds[$key] = $value;
			//log_to("arg.txt",$value);
		}
		else{
...
function try_encode($data) {
	// do our best at encoding a bad user
	$tmp = json_encode($data);
	if($tmp) {return $data;}
	$now =  date("d-m-y H:i:s");
	$rtn ="";
	$x = print_r($data,true);
	//log_to(LOG,"$now [try_encode] -> data to encode $x");
	if(is_array($data)){$fix = mb_strrev($data[3]);} // someone sent an array
	else { $fix =  mb_strrev($data);}
	$fix = mb_str_split($fix);
	//log_to(LOG,"$now we are going to test ".print_r($fix,true));
	foreach($fix as $tmp) {
		// loop it
		$x = json_encode($tmp);
		if(!$x) {$rtn .="?";} //wont encode add a q mark 
		else{$rtn .=$tmp;} // good chr  add it
	}
	//log_to(LOG, "$now [try_encode]->Our encoded string (reversed) is $rtn");
	$rtn = mb_strrev($rtn);
	$tmp = json_encode($rtn);
	if($tmp) {log_to(LOG,"$now [try_encode']-> this does encode $rtn"); return $rtn;}
	else { log_to(LOG,"$now [try_encode]-> holy shit $rtn still does not encode !");}
}
